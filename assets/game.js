...
function potionAmount(p,kind){var max=kind==='hp'?p.hpMax:(kind==='mana'?p.manaMax:p.staminaMax);return Math.max(1,Math.round(max*(0.10+Math.random()*0.05)));}
...
function defaultSave(){
 return {player:{cls:'Einherjar',level:1,xp:0,base:{STR:6,DEX:3,END:9,RUN:2},hp:140,hpMax:140,mana:30,manaMax:30,stamina:100,staminaMax:100,gold:0,weaponBase:5,pet:false,pos:null,lastRegen:Date.now()},maps:{},usedSeeds:{}};
}
...
function moveTo(s,map,id,pos){
 ...
 if(t.drop){
  ...
  var amt=potionAmount(p,kind);var msg=applyPotion(p,kind,amt);
...
function renderBattle(s,idm){
 ...
 if(dead){
  ...
  if(Math.random()<DROP_CHANCE){var dk=DROP_KINDS[Math.floor(Math.random()*3)];var amt=potionAmount(p,dk);var dmsg=applyPotion(p,dk,amt);reward+='<p class="dead">🧪 Looted a '+dk+' potion: '+dmsg+' (auto-collected).</p>';}
